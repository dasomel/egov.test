 ...
 *
 * <h2>요구사항</h2> Spring 2.5 이상 또는 iBATIS sqlmap
 * 2.3.2 이상,<br />
...
public class RefreshableSqlMapClientFactoryBean extends SqlMapClientFactoryBean implements SqlMapClientRefreshable, DisposableBean {
	...
	private void setRefreshable() {
		proxy = (SqlMapClient) Proxy.newProxyInstance(SqlMapClient.class.getClassLoader(),
				new Class[] { SqlMapClient.class, ExtendedSqlMapClient.class }, new InvocationHandler() {
		...
		List<Resource> mappingLocationList = extractMappingLocations(configLocations);

		if (this.mappingLocations != null) {
			mappingLocationList.addAll(Arrays.asList(this.mappingLocations));
		}
		this.mappingLocations = mappingLocationList.toArray(new Resource[0]);
	}

	private List<Resource> extractMappingLocations(Resource[] configLocations) {
		List<Resource> mappingLocationList = new ArrayList<>();
